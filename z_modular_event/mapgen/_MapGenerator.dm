...
/datum/advanced_map_generator/proc/generate_terrain(list/turf/turfs)
	...
	for(var/turf/gen_turf as anything in turfs)
		...
		generate_turf(gen_turf, CHANGETURF_IGNORE_AIR|CHANGETURF_DEFER_CHANGE)
		CHECK_TICK
...
/datum/advanced_map_generator/proc/populate_turfs(list/turf/turfs)
	var/start_time = REALTIMEOFDAY
...
/datum/advanced_map_generator/proc/generate_turf(turf/gen_turf, changeturf_flags)
	SHOULD_NOT_SLEEP(TRUE)
	return

/// Internal proc that actually adds objects to a turf passed to populate_turfs().
/// Should never sleep.
/datum/advanced_map_generator/proc/populate_turf(turf/gen_turf)
	SHOULD_NOT_SLEEP(TRUE)
	return
