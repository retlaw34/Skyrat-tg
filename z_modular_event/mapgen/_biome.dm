...
/datum/adv_biome/proc/generate_turf(turf/gen_turf, area/new_area, changeturf_flags, string_gen)
	...
	var/stored_flags = gen_turf.flags_1 & NO_RUINS
...
/datum/adv_biome/proc/get_turf_type(turf/gen_turf, string_gen)
	return pick_weight(open_turf_types)
...
/datum/adv_biome/proc/populate_turf(turf/gen_turf, list/feature_list, list/mob_list)
	...
	if(feature_spawn_list && prob(feature_spawn_chance) && (a_flags & FLORA_ALLOWED)) //checks the same flag because lol dunno
		var/atom/feature_type = pick_weight(feature_spawn_list)
